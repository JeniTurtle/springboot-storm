...
"""训练预测(datax为x坐标为时间序列,datay为y坐标为时间序列对应的值)"""
class WaveDecompositionBolt(storm.BasicBolt):
  ...
  def train_predict(self,datax,datay,predict_len):
    ...
    train_input_fn = tf.contrib.timeseries.RandomWindowInputFn(reader, batch_size=128, window_size=8)

    estimator = ts_estimators.TimeSeriesRegressor(model=_LSTMModel(num_features=1, num_units=28),optimizer=tf.train.AdamOptimizer(0.001))

    estimator.train(input_fn=train_input_fn, steps=2000)
    evaluation_input_fn = tf.contrib.timeseries.WholeDatasetInputFn(reader)
    evaluation = estimator.evaluate(input_fn=evaluation_input_fn, steps=1)
    # Predict starting after the evaluation
    (predictions,) = tuple(estimator.predict(input_fn=tf.contrib.timeseries.predict_continuation_input_fn(evaluation, steps=predict_len)))
    #观察(times表示时间序列)
  ...
  def process(self, tup):
    ...
    #转字典(转json)
    predict_dict= dict(zip(predict_time,map(lambda x: round(x, 4), predict[0])))
